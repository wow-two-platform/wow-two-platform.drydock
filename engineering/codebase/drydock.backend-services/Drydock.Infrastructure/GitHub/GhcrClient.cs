...
/// <summary>
/// Wraps the GitHub Container Registry (GHCR) v2 API to confirm a published image tag exists.
/// </summary>
/// <remarks>
/// Auth flow per probe:
///   1. Fetch an anonymous GHCR pull token (works for public images) and HEAD the manifest.
///   2. On 401/403, mint a pull token authenticated with the signed-in admin's OAuth token (needs
///      <c>read:packages</c>) via GHCR's Bearer realm, then re-HEAD — covers private images.
    ...
    ILogger<GhcrClient> logger) : IContainerRegistryClient
{
    ...
    private const string BasicAuthUser = "drydock";
    ...
    public async Task<ImageCheck> ImageExistsAsync(string repo, string tag, CancellationToken ct)
    {
        ...
        // First attempt: an anonymous pull token (public images need no admin credentials).
        ...
        // admin's OAuth token (needs read:packages), then re-probe.
        var adminToken = await ResolveAdminTokenAsync();
        if (adminToken is null)
            return ImageCheck.Unauthorized;

        var authenticatedToken = await GetPullTokenAsync(imagePath, adminToken, ct);
    ...
    private async Task<string?> GetPullTokenAsync(string imagePath, string? oauthToken, CancellationToken ct)
    {
        using var request = new HttpRequestMessage(
            HttpMethod.Get, $"https://ghcr.io/token?service=ghcr.io&scope=repository:{imagePath}:pull");

        // For a private image, authenticate the token request with the admin's OAuth token. GHCR validates
        // the token (the Basic username is ignored), then mints a pull-scoped registry bearer.
        if (oauthToken is not null)
        {
            var basic = Convert.ToBase64String(Encoding.ASCII.GetBytes($"{BasicAuthUser}:{oauthToken}"));
    ...
    private async Task<string?> ResolveAdminTokenAsync()
    {
