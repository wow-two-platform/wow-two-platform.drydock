...
public abstract record ProductGetByIdResult
{
    ...
    /// <summary>The product could not be returned — <see cref="ICategorizedFailure.Category"/> maps the status.</summary>
    public sealed record Failure(string ErrorMessage, FailureCategory Category) : ProductGetByIdResult, ICategorizedFailure;
}
