...
public abstract record ProductGetAllResult
{
    ...
    /// <summary>The products could not be listed — <see cref="IDrydockFailure.Category"/> maps the status.</summary>
    public sealed record Failure(string ErrorMessage, FailureCategory Category) : ProductGetAllResult, IDrydockFailure;
}
