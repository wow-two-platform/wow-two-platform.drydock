...
public abstract record ProductVersionStatusResult
{
    ...
    /// <summary>The status could not be returned — <see cref="ICategorizedFailure.Category"/> maps the status.</summary>
    public sealed record Failure(string ErrorMessage, FailureCategory Category) : ProductVersionStatusResult, ICategorizedFailure;
}
