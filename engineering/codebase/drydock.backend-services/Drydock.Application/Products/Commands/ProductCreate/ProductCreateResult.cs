...
public abstract record ProductCreateResult
{
    ...
    /// <summary>The product could not be registered — <see cref="IDrydockFailure.Category"/> maps the status.</summary>
    public sealed record Failure(string ErrorMessage, FailureCategory Category) : ProductCreateResult, IDrydockFailure;
}
